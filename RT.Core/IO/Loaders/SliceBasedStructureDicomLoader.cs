...
namespace RT.Core.IO.Loaders
{
    public class SliceBasedStructureDicomLoader
    {
        ...
        private  DicomFile[] sort(DicomFile[] files)
        {
            ...
            if (files[0].Dataset.GetSingleValue<double>(DicomTag.SliceLocation) != double.MaxValue)
            {
        ...
        private  DicomFile[] sortBySliceLocation(DicomFile[] files)
        {
            ...
            {
                var slicelocationx = x.Dataset.GetValue<double>(DicomTag.SliceLocation, 0);
                var slicelocationy = y.Dataset.GetValue<double>(DicomTag.SliceLocation, 0);
